...
3. **`request_id`** field in proto messages for contract-level traceability
...
| `RequestIdClientInterceptor` | Client interceptor that injects `x-wanaku-request-id` header from MDC |
| `TracingServerInterceptor` | Server interceptor that extracts `x-wanaku-request-id` header and sets MDC + span attribute |
...
3. The `RequestIdClientInterceptor` injects `x-wanaku-request-id` from MDC into request metadata
4. OTel automatically injects `traceparent` (default)
...
1. Extracts `x-wanaku-request-id` from request metadata
